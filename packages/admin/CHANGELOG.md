...
- Enh: Custom side menu via `admin\Module::items` (mdmunir).
...
- Chg: Can customize return of `admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `admin\components\BizRule` (mdmunir).
...
- Chg: Remove `admin\Module::allowActions`. Set access control directly with `admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
